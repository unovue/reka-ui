...
<template>
  <div class="flex w-full flex-col gap-3">
    <div class="inline-flex self-start rounded-lg border border-muted bg-background p-1 shadow-sm">
      <button
        v-for="style in diffStyles"
        :key="style"
        type="button"
        class="h-8 rounded-md px-3 text-sm font-medium capitalize text-muted-foreground transition-colors hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
        :class="diffStyle === style ? 'bg-muted text-foreground shadow-sm' : ''"
    ...
    <MultiFileDiff
      class="block max-h-[360px] w-full overflow-auto rounded-lg border border-muted bg-background text-[13px] shadow-sm"
      :old-file="oldFile"
      :new-file="newFile"
      :options="options"
    />
