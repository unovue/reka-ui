...
5. **SSR = caller-gated, not helper-guarded** — helpers return `globalThis.document` for null anchors; callers gate with `isClient`/`watchEffect` (the existing convention). Do not add per-helper guards that mask mistimed calls.
