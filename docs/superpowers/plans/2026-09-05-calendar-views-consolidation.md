...
- [ ] **5.5** Full verification through the PR's CI (`build.yaml`: type-check, build, vitest, size-limit, docs build). First vitest run surfaced 15 failures across 7 suites — function-valued props unwrapped with `toValue()` (which calls a matcher as a getter), grid watchers flushing a tick late for headless consumers, the `maximumLength` highlight capped at the hovered cell instead of v2's full window, tests that passed `placeholder` (controlled in v3) where `defaultPlaceholder` was meant, and a portal-timing assertion — all fixed on the branch. Calendar size budget raised 17 → 20 kB (the export now carries the month / year adapters and view logic; the full library shrank to ~148 kB of 160).
