...
- [x] **2.7** Harnesses `story/_CalendarViews.vue` (three views + trigger) and `story/_CalendarUnit.vue` (single-view picker); every Calendar and DatePicker story, docs demo and docs snippet moved to `CalendarCell :value` / `CalendarCellTrigger :value` / `CalendarGrid :value`. `CalendarView` / `CalendarViewTrigger` registered in `constant/components.ts` and the namespaced plugin export. Histoire stories for the views: pending.
